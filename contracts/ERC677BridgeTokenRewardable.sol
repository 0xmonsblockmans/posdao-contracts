...
contract Claimable {
    ...
    function claimErc20Tokens(
        ...
        // Interface instance for the ERC20 token
    ...
    function safeTransfer(
        ...
        bytes memory callData = abi.encodeWithSelector(TRANSFER, _to, _value);
        (bool success, bytes memory returnData) = _token.call(callData);

        require(success, "ERC20 transfer failed");

        // Return data is optional but, if provided, must indicate success
        if (returnData.length > 0) {
