...
contract Registry is Owned, IMetadataRegistry, IOwnerRegistry, IReverseRegistry {
    ...
    modifier whenUnreserved(bytes32 _name) {
        require(
            ...
            "Name is reserved or deleted"
        );
        _;
    }

    modifier onlyOwnerOf(bytes32 _name) {
        require(entries[_name].owner == msg.sender, "Caller is not the owner");
        _;
    }

    modifier whenProposed(string memory _name) {
        require(
            entries[keccak256(bytes(_name))].reverse == msg.sender,
            "Not proposed by caller"
        );
        _;
    }

    modifier whenEntry(bytes32 _name) {
        require(
            !entries[_name].deleted && entries[_name].owner != address(0),
            "Entry does not exist or is deleted"
        );
        _;
    }
    ...
    constructor(address _certifierContract, address _owner) {
        require(
            _certifierContract != address(0),
            "Certifier contract address cannot be zero"
        );
        ...
        entries[serviceTransactionChecker].data["A"] = bytes32(
            uint256(uint160(_certifierContract))
        );
    ...
    function transfer(bytes32 _name, address _to)
        ...
        returns (bool)
    {
        require(_to != address(0), "New owner address cannot be zero");
    ...
    {
        if (
            keccak256(bytes(reverses[entries[_name].reverse])) == _name
        ) {
            emit ReverseRemoved(
    ...
    function setData(
        ...
        bytes32 _value
    ) external whenEntry(_name) onlyOwnerOf(_name) returns (bool) {
    ...
    function setAddress(
        ...
        entries[_name].data[_key] = bytes32(uint256(uint160(_value)));
        emit DataChanged(_name, _key, _key);
        return true;
    }
    ...
    function proposeReverse(string calldata _name, address _who)
        external
        whenEntry(keccak256(bytes(_name)))
    ...
    function removeReverse() external {
        require(
            bytes(reverses[msg.sender]).length != 0,
            "Reverse not registered"
        );
        bytes32 nameHash = keccak256(bytes(reverses[msg.sender]));
        emit ReverseRemoved(reverses[msg.sender], msg.sender);
        delete entries[nameHash].reverse;
    ...
    function getAddress(bytes32 _name, string calldata _key)
        ...
        returns (address)
    {
        return address(uint160(uint256(entries[_name].data[_key])));
    }
